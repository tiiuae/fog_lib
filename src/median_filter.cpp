...
MedianFilter::MedianFilter(int buffer_size, double max_valid_value, double min_valid_value, double max_difference, rclcpp::Node& node) {
  ...
  next     = 0;
  m_median = 0;
  RCLCPP_INFO(node.get_logger(), "[MedianFilter]: Initialized, buffer size: %d", buffer_size);
}
...
bool MedianFilter::isValid(double input, rclcpp::Node &node) {
  ...
  if (std::isfinite(value)) {
    ...
    RCLCPP_WARN_THROTTLE(node.get_logger(), *node.get_clock(), 1.0, "[MedianFilter]: Received value is not a finite number!");
    return false;
  }
}
