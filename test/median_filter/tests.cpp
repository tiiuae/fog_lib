...
TEST(TESTSuite, constraints)
{
  ...
  fog_lib::MedianFilter fil(bfr_len, min, max, max_diff);
...
{
  constexpr size_t bfr_len = 5;
  fog_lib::MedianFilter fil(bfr_len);
...
TEST(TESTSuite, copy)
{
  ...
  {
    // move constructor
    fog_lib::MedianFilter fil2(std::move(fil));
    EXPECT_TRUE(fil2.full());
    EXPECT_EQ(fil2.median(), 3);
  }
